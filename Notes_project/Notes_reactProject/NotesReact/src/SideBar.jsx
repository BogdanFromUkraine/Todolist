...
export const SideBar = observer(({setGroupId}) => 
{
    ...
    const {groups, get_Notes_From_Group, get_User_Data, get_All_Group, user, groupData, userRole} = useStores();
    ...
    useEffect(()=> 
        {
            async function getUserData () 
            {
                ...
                var userIsGroups = roles.role === "Admin" 
                ? groups  // Адмін бачить всі елементи
                : groups.filter(item => groupsOfUser.group.map(Number).includes(item.groupId)); // Фільтрація по groupId
